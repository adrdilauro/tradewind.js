(function(w) {
    function TradeWind() {
        var temporaryAnimation, temporaryFinal, temporaryPrestyles, padding = 100, utils = {
            property: {
                unspecified: 'all',
                prefixes: [
        ...
        function applyAnimationCss(instructions) {
            ...
            for (var i = 0; i < instructions.length; i++) {
                for (var j = 0; j < 4; j++) {
                    for (var k = 0; k < 4; k++) {
                        applyCss(instructions[i][0], utils[instructions[i][1][j][0]].prefixes[k], instructions[i][1][j][1]);
                    }
                }
        ...
        function extractRule(property, rule) {
            if (!rule) {
                return utils[property].unspecified;
            } else {
                return rule;
            }
        }
        ...
        function parseInstructions(instructions, locals) {
            var response = [];
            var currIndex = 0;
            for (var i = 0; i < instructions.length; i++) {
        ...
        function apply(instructions, callback, locals) {
            var finalPadding;
        ...
        this.run = function(instructions, callback) {
            var locals = initialize();
            var parsedInstructions = parseInstructions(instructions, locals);
            if (locals.preStyling) {
                applyPreStylingCss(parsedInstructions);
                setTimeout(function() {
                    apply(parsedInstructions, callback, locals);
                }, tradeWind.padding);
            } else {
                apply(parsedInstructions, callback, locals);
            }
